  ...
  async store ({request, response, auth}){
    try{
      ...
      const payments = await Payments.findBy('idPersonal', id)      
        ...
        /*        const alunos = await Database
        .select("alunosstaffs.id",'alunosstaffs.chavedeacesso','alunosstaffs.idAluno','users.email','users.nome','users.telefone')
        .from('alunosstaffs')
        .innerJoin('users', 'users.id', 'alunosstaffs.idAluno')
        ...
        .from('alunosstaffs')
        .innerJoin('users', 'users.id', 'alunosstaffs.idAluno')
        .where({'alunosstaffs.idPersonal': staff.id, 'alunosstaffs.status': 1}) 
        .limit((plan.rows[0].quantifyStudents - alunos.length))  */
...
//                const quantifyPendingStudents = pendingStudents.length > 0 ? ((alunos.length >= pendingStudents.length) ? (alunos.length - pendingStudents.length) : (pendingStudents.length - pendingStudents.length - alunos.length) ) : 0 
